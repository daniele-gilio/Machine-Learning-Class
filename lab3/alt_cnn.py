...
x_train=pvml_cnn.forward(image.imread(train_file[0])[None, :, :, :]/255)[-6]
y_train=np.zeros(train_size)
for c in classes:
    if c in train_file[0]:
        y_train[0]=classes.index(c)

x_test=pvml_cnn.forward(image.imread(test_file[0])[None, :, :, :]/255)[-6]
...
for f in train_file[1:]:
    x_train=np.append(x_train, pvml_cnn.forward(image.imread(f)[None, :, :, :]/255)[-6])
    for c in classes:
        if c in f:
...
for f in test_file[1:]:
    x_test=np.append(x_test, pvml_cnn.forward(image.imread(f)[None, :, :, :]/255)[-6])
    for c in classes:
        if c in f:
...
slp=mlp.MLP([x_train.shape[1], 12])
...
plt.ioff()
if aug==False:
    slp.save("alt_slp_params.npz")
else:
    slp.save("alt_slp_aug_params.npz")
test_labels=slp.inference(x_test)[0]
