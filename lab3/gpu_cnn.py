...
class CNN:
    ...
    def __init__(self, channels=None, kernel_sz=None, strides=None):
        ...
        self.weights = [
            cp.random.randn(k, k, m, n) * cp.sqrt(2.0 / (k * k * m))
            for m, n, k in zip(channels[:-1], channels[1:], kernel_sz)
        ]
        # Biases are zero-initialized
        self.biases = [cp.zeros(m) for m in channels[1:]]
    ...
    def backward(self, Y, activations):
        ...
        d[cp.arange(Y.shape[0]), Y] -= 1  # Subtract the one-hot vectors
...
def _im2col(X, kh, kw, sh, sw):
    ...
    Y = cp.lib.stride_tricks.as_strided(X, shape=shape, strides=strides,
                                        writeable=False)
