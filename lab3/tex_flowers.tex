...
\end{table}
The SLP was trained for $500$ epochs with a batch size of $50$. The learning rate was set at $10^{-3}$ and updated at each epoch\footnote{As always with update it with $\text{LR}=\text{LR}_0 \mathbin{/} \sqrt{t}$, where $t$ is the current epoch number.}. Table \ref{tab:ft-pvmlnet} seems to indicate that the further back we go into PVMLNet the better the result. We were not able to prove this result due to hardware limitations (we ran out of memory during layer $-7$ testing). Looking at Figure \ref{fig:lacc} we can see that we probably would have encountered the law of diminishing returns.
\begin{figure}[!ht]
\centering
\includegraphics[width=0.4\linewidth]{layers_acc.png}
\caption{Layers Accuracy}
\label{fig:lacc}
\end{figure} 
