...
def logreg_train(X, Y, lr=1e-3, steps=1000, init_w=None, init_b=0):
    ...
    for step in range(steps):
        ...
        grad_w = np.multiply(np.matmul((P - Y), X), 1/m)
        grad_b = (P - Y).mean()
        w -= np.multiply(lr ,grad_w)
        b -= lr * grad_b
        if (step+1)%100==0:
...
def logreg_l2_train(X, Y, lambda_, lr=1e-3, steps=1000, init_w=None,
    ...
    for step in range(steps):
        lr=lr0/(step+1)**0.5
        P = logreg_inference(X, w, b)
        grad_w = np.multiply(np.matmul((P - Y), X), 1/m)+2*lambda_*w
        grad_b = (P - Y).mean()
        w -= np.multiply(lr ,grad_w)
        b -= lr * grad_b
        if step%1000==0:
...
def logreg_l1_train(X, Y, lambda_, lr=1e-3, steps=1000, init_w=None, init_b=0, lr0=1):
    ...
    for step in range(steps):
        lr=lr0/(step+1)**0.5
        P = logreg_inference(X, w, b)
        grad_w = np.multiply(np.matmul((P - Y), X), 1/m)+lambda_*np.sign(w)
        grad_b = (P - Y).mean()
        w -= np.multiply(lr ,grad_w)
        b -= lr * grad_b
        if step%100==0:
